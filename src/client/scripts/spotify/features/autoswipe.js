...
export class AutoSwipe {
    constructor(appState, player, uiUtils) {
    ...
    async autoSwipeLoop() {
        ...
        try {
            while (this._shouldContinueAutoSwipe()) {
                ...
                // RevealAtEnd functionality - reveal current track before moving to next
                const userOptions = utils.getUserOptions();
                const revealAtEnd = userOptions.RevealAtEnd || false;
                const revealDuration = userOptions.RevealDuration || CONFIG.DEFAULT_REVEAL_DURATION;
                const revealOnlyUndiscovered = userOptions.RevealOnlyUndiscovered !== false; // true by default

                console.log('🎭 Options utilisateur:', userOptions);
                console.log('🎭 RevealAtEnd activé:', revealAtEnd);
                console.log('🎭 RevealOnlyUndiscovered:', revealOnlyUndiscovered);

                if (revealAtEnd) {
                    const currentTrack = this.appState.getCurrentTrack();
                    console.log('🎭 Track actuelle pour révélation:', currentTrack);

                    if (currentTrack) {
                        // Vérifier si la chanson a été découverte
                        const isDiscovered = currentTrack.discovered || false;
                        console.log('🎭 Chanson découverte:', isDiscovered);

                        // Révéler si: (mode tout révéler) OU (mode sélectif ET pas découverte)
                        const shouldReveal = !revealOnlyUndiscovered || !isDiscovered;

                        if (shouldReveal) {
                            console.log('🎭 RevealAtEnd activé - Révélation de la piste');
                            console.log('🎭 Durée de révélation:', revealDuration, 'ms');

                            // Reveal current track info
                            await utils.revealTrackInfo(currentTrack, revealDuration);
                        } else {
                            console.log('🎭 Chanson déjà découverte et mode sélectif activé, pas de révélation');
                        }
                    } else {
                        console.warn('🎭 Aucune track actuelle trouvée pour la révélation');
                    }

                    // Check if autoswipe should still continue after reveal
                    if (!this.appState.autoSwipe.enabled || this.appState.autoSwipe.status !== 'running') {
                        break;
                    }
                } console.log(`⏭️ AutoSwipe: passage à la piste ${this.appState.currentIndex + 1}`);

                // Use player to go to next track
                const hasNext = this.player.nextTrack();
                if (!hasNext) {
                    break;
                }
